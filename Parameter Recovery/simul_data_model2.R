...
simul_pars = data.frame(phi_pos = rtnorm(num_subjs, 0.999, 0.05, a = 0, b = 1),
                        eta_pos = rtnorm(num_subjs, 0.99, 0.15, a = 0),
                        gam_pos = rtnorm(num_subjs, 0.02, 0.10, a = 0, b = 2),
                        tau_pos = rtnorm(num_subjs, 0.5, 0.34, a = 0),
                        subjID = 1:num_subjs)
...
for( i in 1:num_subjs) {
  ...
  for( t in 1:num_trials) {
    p = (phi_pos + eta_pos * (n_total - n_succ )) / (1+ eta_pos * n_total)
    for(l in 1:I) {
      curUtil[l] =(1 - p^l) * (I - l)^gam_pos
    }
...
write.table(all_data, file = "simul_data_model2.txt", row.names = FALSE, col.names = TRUE, sep = "\t")
